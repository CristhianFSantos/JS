...
function checkExam(examAnswers, studentAnswers) {
  const checkNote = (questionNote, index) => {
    if (questionNote == "") return 0;
    return questionNote === examAnswers[index] ? 4 : -1;
  };

  const notes = studentAnswers.map(checkNote);

  const calculateNotes = notes.reduce((accumulator, currentValue) => {
    return accumulator + currentValue;
  }, 0);

  return calculateNotes > 0 ? calculateNotes : 0;
}
