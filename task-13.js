...
function calculateAge(birthYear, targetYear) {
  const differenceYears = targetYear - birthYear;

  const messageOptions = [
    {
      condition: (value) => value < 0 && value == -1,
      message: `Você nascerá daqui ${-differenceYears} ano.`,
    },
    {
      condition: (value) => value > 0 && value == 1,
      message: `Você tem ${differenceYears} ano de idade.`,
    },
    {
      condition: (value) => value === 0,
      message: "Você nasceu neste mesmo ano!",
    },
    {
      condition: (value) => value < 0,
      message: `Você nascerá daqui ${-differenceYears} anos.`,
    },
    {
      condition: (value) => value > 0,
      message: `Você tem ${differenceYears} anos de idade.`,
    },
  ];

  const optionMeetsTheCondition = messageOptions.find((optionObj) =>
    optionObj.condition(differenceYears)
  );

  return optionMeetsTheCondition.message;
}
